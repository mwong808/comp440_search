    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append((tuple(temp_state), action, 1))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # print("fg", foodGrid.asList())
    # print("walls", problem.walls)
    if problem.isGoalState(state):
        return 0
    #try max
    answer = -1
    for new_position in foodGrid.asList():
        dist = mazeDistance(position, new_position, problem.startingGameState)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        food_tuples = food.asList()
        curr_pos = gameState.getPacmanPosition()
        # print("food", food_tuples, "start")
        min_dist = 99999999
        closest = (None,None)
        for food_tuple in food_tuples:
            dist = mazeDistance(curr_pos,food_tuple,gameState)
            if dist < min_dist:
                min_dist = dist
                closest = food_tuple
